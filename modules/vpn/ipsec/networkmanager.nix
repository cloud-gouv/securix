  ...
  mkIPsecConnectionProfile =
    ...
    let
      resolvedLocalIdentity =
        if lib.isAttrs local-identity then local-identity.${profileName} or mkIf( method != "cert-on-security-token" ) (email) else local-identity;
    in
      ...
      "Si aucune gateway dans le tunnel `${profileName}` n'est spécifié, alors le mode config d'IPsec doit etre configuré pour la configuration IP automatique.";
    {
      ...
      ipv4 = {
        method = if localSubnet == "%any" then "auto" else "manual";
