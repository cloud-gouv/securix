...
{ pkgs, vpnProfiles, operators, config, lib, ... }:
let
  cfg = config.securix.vpn;
  inherit (lib) mkIf mkEnableOption mkOption types nameValuePair concatMapAttrs concatStringsSep listToAttrs mapAttrs mapAttrsToList;
  mkIPsecConnectionProfile = operatorName: { username, email, bit, ... }: profileName: { endpoint, remote-identity, method, ike, esp, remoteSubnets, localSubnet, gateway ? null, mkPasswordVariable ? null, mkAddress ? null, ... }:
  assert lib.assertMsg (mkPasswordVariable == null -> method != "psk") "Si la méthode PSK est spécifié pour le tunnel `${profileName}`, une façon de récupérer le mot de passe via une variable d'environnement doit etre spécifié.";
  assert lib.assertMsg (mkAddress == null -> localSubnet == "%any") "Si aucune méthode de construction d'adresse IP dans le tunnel `${profileName}` n'est spécifié, alors le mode config d'IPsec doit etre configuré pour la configuration IP automatique.";
  assert lib.assertMsg (gateway == null -> localSubnet == "%any") "Si aucune gateway dans le tunnel `${profileName}` n'est spécifié, alors le mode config d'IPsec doit etre configuré pour la configuration IP automatique.";
  {
    connection = {
    ...
    vpn-secrets = mkIf (method == "psk") {
      password = mkPasswordVariable operatorName;
    };
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    security.sudo.extraRules = [
      { 
        ...
        commands = [
          {
            command = "/run/current-system/sw/bin/pkill charon-nm"; options = [ "NOPASSWD" ];
          }
        ];
      }
    ];
  ...
  nixpkgs.overlays = [
    ...
    networking.networkmanager.ensureProfiles.environmentFiles = mapAttrsToList (name: _: config.age.secrets.${name}.path) cfg.pskSecretsPaths;
    networking.networkmanager.ensureProfiles.profiles = concatMapAttrs (op: opCfg:
      listToAttrs (map (profileName: 
        nameValuePair "${op}-${profileName}" (mkIPsecConnectionProfile op opCfg profileName vpnProfiles.${profileName})
      ) opCfg.allowedVPNs)
    ) operators;
  };
}
