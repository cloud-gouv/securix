...
{ pkgs, config, lib, ... }:
let
  inherit (lib) types mkOption mkEnableOption filterAttrs mapAttrs' nameValuePair;
  ...
  mkTunnelService = name: { description, vpn, target, localPort, remoteAddress, remotePort, ... }: nameValuePair "ssh-tunnel-to-${name}" {
    inherit description;
    after = [ "network.target" ];

    path = [ pkgs.libnotify pkgs.networkmanager ];
...
in
{
  ...
  config = {
    # One per operator.
    systemd.user.services = mapAttrs' mkTunnelService (filterAttrs (_: { enable, ... }: enable) tunnels);
  };
}
