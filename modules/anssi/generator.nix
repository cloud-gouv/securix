{
  ...
  generateOptionHierarchy = rulesets:
  let
    levelMapping = {
      "minimal" = 0;
      "intermediary" = 1; 
    ...
    makeRuleOption = { lib, config, pkgs, ... }: ruleId: rule:
      {
        ...
        enable = lib.mkOption {
          ...
          default =
            ...
            && (((rule.architectures or [ ]) != []) -> lib.elem pkgs.stdenv.hostPlatform.linuxArch rule.architectures)
        ...
        exceptionRationale = lib.mkOption {
          ...
          default = if config.security.anssi.exceptions ? ${rule.name} then config.security.anssi.exceptions.${ruleId}.rationale else null;
          readOnly = true;
    ...
    ruleOptions = moduleArgs: moduleArgs.lib.mapAttrs (ruleId: rule: makeRuleOption moduleArgs ruleId rule) rulesets;
    ruleEnabled = config: ruleId: config.security.anssi.rules.${ruleId}.enable;
    enabledRuleConfigs = moduleArgs:
      moduleArgs.lib.mapAttrsToList (ruleId: rule:
      let
        complexConfigTerm = cfg: moduleArgs.lib.mkMerge (moduleArgs.lib.mapAttrsToList (implName: _:
