...
in
{
    ...
    in
  {
    ...
    config = { ... }: {
      boot.kernel.sysctl = sysctls;
    };

    checkScript = pkgs: pkgs.writeShellScript "check-R9" ''
  ...
  R10 = {
    ...
    checkScript = pkgs: pkgs.writeShellScript "check-R10" ''
  ...
  R11 = {
    ...
    checkScript = pkgs: pkgs.writeShellScript "check-R11" ''
  ...
  in
  {
    ...
    checkScript = pkgs: pkgs.writeShellScript "check-R12" ''
  ...
  in
  {
    ...
    checkScript = pkgs: pkgs.writeShellScript "check-R13" ''
      ${mkSysctlChecker pkgs.lib sysctls}
    '';
  };
