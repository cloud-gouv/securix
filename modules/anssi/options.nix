...
let
  ...
  determineExclusionReason = ruleId: rule: 
    if !cfg.rules.${ruleId}.enable then
      ...
      let
          ...
          architectureExclusion =  if (rule.architectures or [ ]) != [ ] then !lib.elem pkgs.stdenv.hostPlatform.linuxArch rule.architectures else false;
      in
        if tagExclusions != [] then
          { reason = "excluded by tag ${lib.concatStringsSep ", " tagExclusions}"; via = "tag"; exclusionTags = tagExclusions; }
        else if categoryExclusion then
          { reason = "out of scope for category '${cfg.category}' (${rule.category})"; via = "category"; requiredCategory = rule.category; }
        else if levelExclusion then
          { reason = "out of scope for level ${cfg.level} < ${rule.severity}"; via = "level"; requiredLevel = rule.severity;}
        else if architectureExclusion then
          { reason = "out of scope for architecture '${pkgs.stdenv.hostPlatform.linuxArch}'"; via = "architecture"; requiredArchitectures = rule.architectures or [ ]; }
        else if lib.hasAttr ruleId cfg.exceptions then
          { reason = "excluded by explicit exception: ${cfg.exceptions.${ruleId}.rationale}"; via = "exception"; rationale = cfg.exceptions.${ruleId}.rationale; }
        else
          { reason = "could not understand why this rule was excluded, the user must have set the `enable = false;` forcibly via the module system bypassing the governance system. This is a non-compliant."; via = "unknown"; }
    else
      null;

  generateRuleEntry = ruleId: rule: {
  ...
  complianceReport = {
    "org.securix.anssi-compliance.v1" = {
      ...
      exclusions = lib.mapAttrs determineExclusionReason (lib.filterAttrs (ruleId: _: !cfg.rules.${ruleId}.enable) rulesets);
    };
  };
in
{
  ...
  options.security.anssi = {
    ...
    level = mkOption {
      type = types.enum [ "minimal" "intermediary" "reinforced" "high" ];
      default = "minimal";
      description = "Select desired ANSSI compliance level.";
    };

    category = mkOption {
      type = types.enum [ "base" "client" "server" ];
      default = "base";
    ...
    exceptions = mkOption {
      type = types.attrsOf (types.submodule ({ ... }: { options.rationale = mkOption { type = types.lines; }; }));
      default = { };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    pkgs.writeShellScript "anssi-nixos-compliance-check" ''
      ...
      ${lib.concatStringsSep "\n" (lib.mapAttrsToList mkCheckSingularRule (lib.filterAttrs (_: rule: rule.enable) config.security.anssi.rules))}
