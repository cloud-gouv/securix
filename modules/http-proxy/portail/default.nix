    ...
    systemd.services.portail-dynamic-updates =
      ...
      in
      {
        ...
        serviceConfig = {
          ...
          Restart = "on-failure";
          RestartSec = 5;
          LoadCredential = mapAttrsToList (id: { remotePath, ... }: "${id}:${remotePath}") (
            filterAttrs isDynamic config.securix.automatic-http-proxy.proxies
          );
        };
        script =
          let
    ...
    services.portail = {
      ...
      settings = {
        ...
        backends = mapAttrs (
          ...
          // optionalAttrs (target-address != null) { inherit target-address; }
        ) cfg.downstreams;
        rpc = {
