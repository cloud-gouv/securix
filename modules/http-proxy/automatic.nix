...
let
  ...
  mkDefinitionAssert =
    ...
    }:
    {
      ...
      message = ''
        Upstream `${name}` is defined as a static proxy but its remote information is not specified (${builtins.toJSON remote})
...
in
{
  ...
  config = mkIf cfg.enable {
    assertions = mapAttrsToList (
      name: { definition, remote, ... }: mkDefinitionAssert { inherit name definition remote; }
    ) cfg.proxies;
    ...
    securix.http-proxy = {
      ...
      downstreams = mapAttrs (
        _:
        { definition, remote, ... }:
        if definition == "static" then "${remote.address}:${toString remote.port}" else null
      ) cfg.proxies;
    };
